...
def calc_normalize_scales(data, type=None):
	"""
	...
	if type == "vector":
		...
		#new 95th percentile
		norm = np.sqrt(data[0].flatten()**2 + data[1].flatten()**2 + data[2].flatten()**2)
		...
		return np.percentile(np.abs(data.flatten()), 95)
...
def main():
	...
	with open("./data/sim_metadata.json", "r") as f:
		...
		for sim in range(num_simulations):
			...
			print("Flow reading from : ", start_index," till : ", start_index +num_samples-1 )
			...
			data = membrane_data[3].transpose(1,0,2) 
			
			velocity_stats = calc_normalize_scales(membrane_data[1].transpose(1,0,2),type="vector")
			pressure_stats = calc_normalize_scales(membrane_data[2].transpose(1,0),type="scalar")
			force_stats = calc_normalize_scales(membrane_data[3].transpose(1,0,2),type="vector")

			sim_stats_membrane = {
				"sim_id": sim_params["sim_id"],
				"velocity_scale": velocity_stats,
				"force_scale": force_stats,
				"pressure_scale": pressure_stats,
			...
			flow_data = flow_data[start_index:start_index+num_samples]

			velocity = flow_data.transpose(1,0,2)[0:3]
			velocity[0] -= 1 #As the incoming flow is set to 1 in the simulations

			velocity_stats = calc_normalize_scales(velocity,type="vector")
			pressure_stats = calc_normalize_scales(flow_data.transpose(1,0,2)[3],type="scalar")
			sim_stats_flow = {
				"sim_id": sim_params["sim_id"],
				"velocity_scale": velocity_stats,
				"pressure_scale": pressure_stats,
